...
Entertainment-слой: странные, смешные и неожиданно популярные репозитории.

### Карточки

| Блок | Что | Лимит |
|------|-----|-------|
| **Что это?** | Одно предложение — что делает репо | ≤140 символов, `line-clamp-2` |
| **Почему интересно?** | Уникальная причина отбора (рост, категория, идея) | ≤220 символов, `line-clamp-3` |

- Детерминированная генерация: `lib/weird-card-copy.ts` (валидация, anti-dedupe)
- AI по кнопке: `POST /api/weird/content` → `lib/weird-ai.ts`
- Скоринг и отбор: `lib/weird.ts`, категории в `lib/weird-constants.ts`

### UI

- Фильтры: Most Weird, Fastest Growing, Most Starred, Most Discussed
- Категории: pets, humor, useless, retro, AI oddities, visual, memes, why?
- **Weird Find of the Week** — featured-карточка с AI-постами (LinkedIn / Telegram)
...
lib/weird.ts
lib/weird-card-copy.ts
lib/weird-ai.ts
lib/weird-what-is-this.ts
...
- **`weirdFindOfTheWeek`** (опционально) — entertainment-рубрика для Радара:
  `whatIsIt`, `whyInteresting`, `telegramTitle`, `telegramPost`, метрики, `weirdScore`
...
- [ ] `/weird` — карточки с разными «Что это?» и «Почему интересно?»
