...
| | |
|---|---|
| Цепочка | Отчёт → **most_surprising_insight** → reasoning map → key insight → **естественная проза** |
| Длина | 200–600 слов, цель 250–450 |
| Reasoning | `linkedin-reasoning-safety.ts` — observation / interpretation / hypothesis |
| Качество | `lib/linkedin-post-quality.ts` — без section labels, repo evidence, overclaim check |
| Модули | `lib/linkedin-surprising-insight.ts`, `lib/linkedin-post-evidence.ts`, `lib/linkedin-reasoning-safety.ts` |
...
lib/weird.ts
lib/linkedin-reasoning-safety.ts
