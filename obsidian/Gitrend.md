...
- Лаунчер добавляет **Git** в PATH (для публикации Radar JSON из UI).
...
> Нужен установленный Git. Если ошибка `ENOENT` — задайте `GIT_EXECUTABLE` в `.env.local` или перезапустите через `GitHub-Trends-Start.bat`.
