...
- **💼 LinkedIn Post** — синтез полного отчёта (EN/RU, Copy, Regenerate, 200–450 слов)
...
| | |
|---|---|
| Генерация | Полный отчёт → key insight → пост (не пересказ категории) |
| Длина | 200–600 слов, цель 250–450 |
| Качество | `lib/linkedin-post-quality.ts` — reasoning, repo evidence, без generic-фраз |
